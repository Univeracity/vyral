...
/// <summary>
/// Firestore and Cloud Tasks implementation of the portable execution runtime boundary. Handler
/// code runs in the Cloud Run worker selected by the dispatcher; this adapter owns durable state
/// and never executes a handler in the API process.
/// </summary>
public sealed class GoogleCloudExecutionRuntimeAdapter : IExecutionRuntimeAdapter, IExecutionRuntimeMaintenance, IExternalExecutionWorkerRuntime
{
    private readonly IGoogleCloudExecutionStateStore _state;
    ...
    public GoogleCloudExecutionRuntimeAdapter(
        IGoogleCloudExecutionStateStore state,
        IExecutionRunDispatcher dispatcher,
        GoogleCloudExecutionRuntimeOptions? options = null,
        IObjectStore? artifactObjectStore = null)
    {
        _state = state ?? throw new ArgumentNullException(nameof(state));
        _dispatcher = dispatcher ?? throw new ArgumentNullException(nameof(dispatcher));
        Options = options ?? new GoogleCloudExecutionRuntimeOptions();
        ...
        Adapter = new ExecutionRuntimeAdapterDescriptor
        {
            ...
            RuntimeKind = "google.firestore-cloud-tasks",
            DisplayName = "Google Firestore and Cloud Tasks execution runtime",
            Version = "0.2.0",
            Capabilities =
            {
            ...
            Metadata =
            {
                ...
                ["workerTarget"] = "cloud_run"
            }
        };
        ExecutionContractValidator.ValidateAdapterDescriptor(Adapter, Options.Limits);
    }

    public GoogleCloudExecutionRuntimeOptions Options { get; }
    ...
    public void RegisterExternalHandler(ExecutionHandlerDescriptor handler)
    {
        ...
        {
            throw new InvalidOperationException($"External execution handler '{handlerId}' requires an explicit Google worker route.");
        }
    ...
    public async Task<ExecutionRun> StartRunAsync(ExecutionRunRequest request, CancellationToken ct = default)
    {
        ...
        else if (externalHandler is null)
        {
            ...
            run.Error = $"Execution handler '{handlerId}' is registered in the API process but has no configured external Google worker route.";
            run.CompletedAtUtc = now;
            run.DurationMs = 0;
        }
    ...
    public async Task<ExecutionRun?> CancelRunAsync(string runId, CancellationToken ct = default)
    {
        ...
        if (run is null || ExecutionRunStatuses.IsTerminal(run.Status)) return run;
        ...
        if (run.Status is ExecutionRunStatuses.Queued or ExecutionRunStatuses.Waiting)
        {
            ...
            run.DurationMs = (run.CompletedAtUtc.Value - (run.StartedAtUtc ?? run.CompletedAtUtc.Value)).TotalMilliseconds;
            await _state.UpsertRunAsync(run, ct);
            await _state.DeleteWaitAsync(run.Id, ct);
            await AppendAsync(run, ExecutionEventTypes.RunCancellationRequested, "Execution cancellation requested.", ct);
            await AppendAsync(run, ExecutionEventTypes.RunCompleted, run.Error, ct);
            return run;
        }
    ...
    public async Task<ExecutionExternalWorkerLease?> LeaseNextRunAsync(ExecutionExternalWorkerLeaseRequest request, CancellationToken ct = default)
    {
        ...
        foreach (var candidate in candidates.OrderBy(run => run.CreatedAtUtc))
        {
            ...
            // A Cloud Tasks delivery can be acknowledged by a concurrent worker before this
    ...
    public async Task<ExecutionExternalWorkerWaitResponse> WaitExternalLeaseAsync(ExecutionExternalWorkerWaitRequest request, CancellationToken ct = default)
    {
        ...
        run = await _state.SuspendExternalRunUnderLeaseAsync(
            lease.LeaseKey,
            lease.OwnerId,
            new GoogleCloudExecutionWait { RunId = run.Id, Kind = kind, Name = request.Name, FireAtUtc = fireAt, Timer = timer },
            ct);
        ...
        if (kind == ExecutionExternalWorkerWaitKinds.ExternalEvent)
        {
            ...
            if (racedEvent is not null)
            {
                ...
                await _state.PutWaitOutcomeAsync(run.Id, new GoogleCloudExecutionWait { RunId = run.Id, Kind = kind, Name = request.Name }, racedOutcome, ct);
                await _state.DeleteWaitAsync(run.Id, ct);
                var resumed = await _state.GetRunAsync(run.Id, true, ct);
                if (resumed?.Status == ExecutionRunStatuses.Waiting)
                {
                    resumed.Status = ExecutionRunStatuses.Queued;
                    resumed.ScheduledAtUtc = null;
                    resumed.CurrentStep = null;
                    resumed.UpdatedAtUtc = DateTime.UtcNow;
                    await _state.UpsertRunAsync(resumed, ct);
                }

                await DispatchAsync(run, ExecutionDispatchReasons.ExternalEvent, null, ct);
            }
        }
    ...
    public async Task<ExecutionExternalEvent> RaiseEventAsync(ExecutionExternalEventRequest request, CancellationToken ct = default)
    {
        ...
        {
            await _state.PutWaitOutcomeAsync(externalEvent.RunId!, wait, new ExecutionWaitResult { Name = wait.Name, Outcome = ExecutionWaitOutcomes.ExternalEvent, Event = externalEvent }, ct);
            await _state.DeleteWaitAsync(externalEvent.RunId!, ct);
            var run = await _state.GetRunAsync(externalEvent.RunId!, true, ct);
        ...
        var eventRun = await _state.GetRunAsync(externalEvent.RunId!, true, ct) ?? throw new InvalidOperationException("Execution event run was not found.");
    ...
    public async Task<ExecutionMaintenanceStatus> GetMaintenanceStatusAsync(CancellationToken ct = default)
    {
        ...
        return new ExecutionMaintenanceStatus
        {
            ...
            CheckedAtUtc = DateTime.UtcNow,
            RetentionScope = "run_owned_firestore",
    ...
    public async Task<ExecutionMaintenanceDispatchReconcileResult> ReconcileDispatchAsync(ExecutionMaintenanceDispatchReconcileRequest request, CancellationToken ct = default)
    {
        ...
        var now = DateTime.UtcNow;
        // A Cloud Tasks delivery may have been acknowledged just before a worker crashed. A
    ...
    private static IReadOnlyDictionary<string, ExecutionProductPolicy> BuildProductPolicies(IReadOnlyList<ExecutionProductPolicy>? policies)
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(policy.ProductId)) throw new InvalidOperationException("Google execution product policy product id is required.");
            ...
            if (!result.TryAdd(policy.ProductId.Trim(), policy)) throw new InvalidOperationException($"Google execution product policy '{policy.ProductId}' is duplicated.");
        }
    ...
    {
        if (Options.MaxActiveRuns <= 0) throw new InvalidOperationException("Google execution max active runs must be positive.");
        if (Options.MaxRetainedTerminalRuns < 0) throw new InvalidOperationException("Google execution max retained terminal runs must be non-negative.");
        if (Options.DefaultListLimit <= 0 || Options.MaxListLimit < Options.DefaultListLimit)
            throw new InvalidOperationException("Google execution list limits are invalid.");
        if (Options.DefaultHistoryLimit <= 0 || Options.MaxHistoryLimit < Options.DefaultHistoryLimit)
            throw new InvalidOperationException("Google execution history limits are invalid.");
        if (Options.MaintenanceScanLimit <= 0) throw new InvalidOperationException("Google execution maintenance scan limit must be positive.");
        if (Options.MaxActiveRuns > Options.MaintenanceScanLimit)
            throw new InvalidOperationException("Google execution max active runs cannot exceed maintenance scan limit.");
        GoogleFirestoreExecutionLimits.Validate(Options.Limits);
        if (_artifactObjectStore is null && Options.Limits.MaxArtifactBytes > Options.Limits.MaxArtifactInlineBytes)
            throw new InvalidOperationException("Google execution artifact limits above the inline ceiling require a configured durable artifact object store.");
        if (_artifactObjectStore is not null && string.IsNullOrWhiteSpace(Options.ArtifactObjectContainer))
            throw new InvalidOperationException("Google execution artifact object storage requires ArtifactObjectContainer.");
    }

    private static IReadOnlyDictionary<string, IExecutionRunDispatcher> BuildWorkerDispatchers(IReadOnlyList<GoogleCloudExecutionWorkerDispatcher>? dispatchers)
    {
        var result = new Dictionary<string, IExecutionRunDispatcher>(StringComparer.Ordinal);
        foreach (var route in dispatchers ?? Array.Empty<GoogleCloudExecutionWorkerDispatcher>())
        {
            if (string.IsNullOrWhiteSpace(route.HandlerId)) throw new InvalidOperationException("Google execution worker route handler id is required.");
            if (route.Dispatcher is null) throw new InvalidOperationException($"Google execution worker route '{route.HandlerId}' requires a dispatcher.");
            if (!result.TryAdd(route.HandlerId.Trim(), route.Dispatcher)) throw new InvalidOperationException($"Google execution worker route '{route.HandlerId}' is duplicated.");
        }
    ...
    private IExecutionRunDispatcher DispatcherFor(ExecutionRun run)
    {
        ...
        throw new InvalidOperationException($"Execution handler '{run.HandlerId}' has no configured Google worker route.");
    }

    private Task DispatchAsync(ExecutionRun run, string reason, DateTime? scheduledAtUtc, CancellationToken ct) =>
        DispatcherFor(run).DispatchAsync(new ExecutionDispatchRequest
        {
            RunId = run.Id,
            Reason = reason,
    ...
    private void EnsureRunBoundary(ExecutionRunRequest request, string handlerId)
    {
        ...
        var scope = request.Scope ?? throw new InvalidOperationException("Execution scope is required when Google product policies are configured.");
    ...
    private async Task<ExecutionArtifact> CreateArtifactAsync(ExecutionRun run, ExecutionArtifactWrite write, CancellationToken ct)
    {
        ...
        if (_artifactObjectStore is null || string.IsNullOrWhiteSpace(Options.ArtifactObjectContainer))
            throw new InvalidOperationException($"Google execution artifacts larger than {Options.Limits.MaxArtifactInlineBytes} bytes require configured durable artifact object storage.");
    ...
    private async Task ResumeDueWaitAsync(string runId, CancellationToken ct)
    {
        ...
        await _state.PutWaitOutcomeAsync(runId, wait, outcome, ct);
        await _state.DeleteWaitAsync(runId, ct);
        var run = await _state.GetRunAsync(runId, true, ct);
        if (run?.Status == ExecutionRunStatuses.Waiting)
        {
            run.Status = ExecutionRunStatuses.Queued;
...
public sealed class GoogleCloudExecutionRuntimeOptions
{
    public string AdapterId { get; init; } = "google-firestore-cloud-tasks";
    public ExecutionRuntimeLimits Limits { get; init; } = GoogleFirestoreExecutionLimits.Default;
    ...
    public IReadOnlyList<GoogleCloudExecutionWorkerDispatcher> WorkerDispatchers { get; init; } = Array.Empty<GoogleCloudExecutionWorkerDispatcher>();
...
/// <summary>
/// Conservative Firestore document limits. Runs retain both payload and result in one document,
/// so each independently embedded value must leave capacity for the complete envelope and indexes.
/// Artifact content is limited to this ceiling unless a durable object store is explicitly wired.
/// </summary>
public static class GoogleFirestoreExecutionLimits
{
    public const int MaxInlineDocumentValueBytes = 262_144;

    public static ExecutionRuntimeLimits Default => new()
    {
        MaxPayloadBytes = MaxInlineDocumentValueBytes,
        MaxResultBytes = MaxInlineDocumentValueBytes,
        MaxStatusDetailsBytes = 65_536,
    ...
    public static void Validate(ExecutionRuntimeLimits limits)
    {
        ...
        if (limits.MaxPayloadBytes > MaxInlineDocumentValueBytes ||
            limits.MaxResultBytes > MaxInlineDocumentValueBytes ||
            limits.MaxCheckpointBytes > MaxInlineDocumentValueBytes ||
            limits.MaxArtifactInlineBytes > MaxInlineDocumentValueBytes)
        {
            throw new InvalidOperationException($"Google Firestore execution embedded values must not exceed {MaxInlineDocumentValueBytes} bytes.");
        }
        if (limits.MaxArtifactBytes < limits.MaxArtifactInlineBytes)
            throw new InvalidOperationException("Google execution max artifact bytes cannot be lower than the inline artifact ceiling.");
    }
}

public sealed class GoogleCloudExecutionWorkerDispatcher
{
...
/// existing Google-host configuration code can migrate without a source break.
/// </remarks>
public sealed class GoogleCloudExecutionProductPolicy : ExecutionProductPolicy
{
}
