...
/// <summary>
/// Runs the provider-neutral external-worker contract against the Google adapter with a
/// deterministic state/dispatch fixture. Firestore and Cloud Tasks are separately validated by
...
/// </summary>
public sealed class GoogleCloudExecutionRuntimeAdapterConformanceTests : ExternalExecutionWorkerRuntimeConformanceTests
{
    [Fact]
    public async Task GoogleExecutionRuntime_MatchesPublishedQualificationProfile()
    {
    ...
    protected override Task<ExternalExecutionWorkerRuntimeFixture> CreateExternalWorkerRuntimeAsync()
    {
        ...
        {
            HandlerId = "conformance.google.external.worker",
            PluginId = "conformance.google.external",
            DisplayName = "Google external worker conformance handler"
        };
        var dispatcher = new CapturingExecutionRunDispatcher();
        var runtime = new GoogleCloudExecutionRuntimeAdapter(
            new InMemoryGoogleCloudExecutionStateStore(),
            dispatcher,
    ...
    [Fact]
    public Task GoogleExternalWorker_AdvertisesExternalExecutionModel() =>
        RunExternalWorker_AdvertisesExternalExecutionModel();

    [Fact]
    public Task GoogleExternalWorker_ReportsArtifactsEventsAndCheckpointsUnderLease() =>
        RunExternalWorker_ReportsArtifactsEventsAndCheckpointsUnderLease();

    [Fact]
    public Task GoogleExternalWorker_DeduplicatesClaimsAndResumesDurableEvents() =>
        RunExternalWorker_DeduplicatesClaimsAndResumesDurableEvents();

    [Fact]
    public Task GoogleExternalWorker_CompletionIsReplaySafeForTheSameLeaseToken() =>
        RunExternalWorker_CompletionIsReplaySafeForTheSameLeaseToken();

    [Fact]
    public Task GoogleExternalWorker_RejectsStaleLeaseEvents() =>
        RunExternalWorker_RejectsStaleLeaseEvents();

    [Fact]
    public async Task GoogleExternalWorker_RunsPortableConsumerPluginUnchanged()
    {
        ...
        var completed = await ExternalExecutionConsumerSample.RunExternalAsync(
            runtime,
            runtime,
            "portable-consumer:google-external");

        await ExternalExecutionConsumerSample.AssertRunShapeAsync(runtime, completed);
    }

    [Fact]
    public async Task GoogleExternalWorker_EnforcesSharedProductPolicyAndRedactsTraceDetails()
    {
        var handler = new ExecutionHandlerDescriptor
        {
            HandlerId = "conformance.google.policy.worker",
            PluginId = "conformance.google.policy",
            DisplayName = "Google product policy handler"
        };
        var dispatcher = new CapturingExecutionRunDispatcher();
        var runtime = new GoogleCloudExecutionRuntimeAdapter(
            new InMemoryGoogleCloudExecutionStateStore(),
            ...
            new GoogleCloudExecutionRuntimeOptions
            {
                ...
                WorkerDispatchers =
                [
                    new GoogleCloudExecutionWorkerDispatcher { HandlerId = handler.HandlerId, Dispatcher = dispatcher }
                ]
            });
    ...
    [Fact]
    public async Task GoogleRuntime_ReconcilesRunnableWorkThroughPortableDispatchSeam()
    {
        var fixture = await CreateExternalWorkerRuntimeAsync();
    ...
    [Fact]
    public async Task GoogleRuntime_EnforcesPublishedOperationalBounds()
    {
        var handler = new ExecutionHandlerDescriptor
        {
            HandlerId = "conformance.google.operational.worker",
            PluginId = "conformance.google.operational",
            DisplayName = "Google operational-policy worker"
        };
        var dispatcher = new CapturingExecutionRunDispatcher();
        var runtime = new GoogleCloudExecutionRuntimeAdapter(
            new InMemoryGoogleCloudExecutionStateStore(),
    ...
    [Fact]
    public async Task GoogleRuntime_AtomicallyReservesIdempotencyKeysAndDispatchesOnce()
    {
        var handler = new ExecutionHandlerDescriptor
        {
            HandlerId = "conformance.google.idempotency.worker",
            PluginId = "conformance.google.idempotency",
            DisplayName = "Google idempotency worker"
        };
        var dispatcher = new CapturingExecutionRunDispatcher();
        var state = new InMemoryGoogleCloudExecutionStateStore();
        var runtime = new GoogleCloudExecutionRuntimeAdapter(
            state,
            dispatcher,
            new GoogleCloudExecutionRuntimeOptions
            {
                WorkerDispatchers = [new GoogleCloudExecutionWorkerDispatcher { HandlerId = handler.HandlerId, Dispatcher = dispatcher }]
            });
        runtime.RegisterExternalHandler(handler);
    ...
    [Fact]
    public async Task GoogleRuntime_OffloadsLargeArtifactsAndRejectsOversizeInlineArtifactsBeforeStateStorage()
    {
        var handler = new ExecutionHandlerDescriptor
        {
            HandlerId = "conformance.google.storage.worker",
            PluginId = "conformance.google.storage",
            DisplayName = "Google storage worker"
        };
        var dispatcher = new CapturingExecutionRunDispatcher();
        var state = new InMemoryGoogleCloudExecutionStateStore();
        ...
        var runtime = new GoogleCloudExecutionRuntimeAdapter(
            ...
            new GoogleCloudExecutionRuntimeOptions
            {
                Limits = GoogleFirestoreExecutionLimits.WithArtifactOffload,
        ...
        var artifact = await runtime.PutExternalLeaseArtifactAsync(new ExecutionExternalWorkerArtifactRequest
        {
            ...
            Artifact = new ExecutionArtifactWrite
            {
                ...
                Text = new string('x', GoogleFirestoreExecutionLimits.MaxInlineDocumentValueBytes + 1)
            }
        });
        ...
        var inlineOnly = new GoogleCloudExecutionRuntimeAdapter(
            new InMemoryGoogleCloudExecutionStateStore(),
            dispatcher,
            new GoogleCloudExecutionRuntimeOptions
            {
    ...
    [Fact]
    public async Task GoogleRuntime_EnforcesFirestoreSafePayloadResultAndCheckpointLimitsBeforePersistence()
    {
        var handler = new ExecutionHandlerDescriptor
        {
            HandlerId = "conformance.google.inline-limits.worker",
            PluginId = "conformance.google.inline-limits",
            DisplayName = "Google inline limits worker"
        };
        var dispatcher = new CapturingExecutionRunDispatcher();
        var runtime = new GoogleCloudExecutionRuntimeAdapter(
            new InMemoryGoogleCloudExecutionStateStore(),
        ...
        runtime.RegisterExternalHandler(handler);
        var justUnder = new string('x', GoogleFirestoreExecutionLimits.MaxInlineDocumentValueBytes - 2);
        ...
        {
            HandlerId = handler.HandlerId,
            Payload = JsonValue.Create(new string('x', GoogleFirestoreExecutionLimits.MaxInlineDocumentValueBytes + 1))
        }));
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() => runtime.CheckpointExternalLeaseAsync(new ExecutionExternalWorkerCheckpointRequest
        {
            ...
            LeaseToken = lease.LeaseToken,
            WorkerId = lease.WorkerId,
            Checkpoint = new ExecutionCheckpointWrite { Key = "too-large", Content = JsonValue.Create(new string('x', GoogleFirestoreExecutionLimits.MaxInlineDocumentValueBytes + 1)) }
        }));
    ...
    private sealed class InMemoryGoogleCloudExecutionStateStore : IGoogleCloudExecutionStateStore
    {
        ...
        private readonly Dictionary<string, GoogleCloudExecutionWait> _waits = new(StringComparer.Ordinal);
        ...
        public Task<GoogleCloudExecutionRunCreation> CreateRunAtomicallyAsync(
            ExecutionRun run,
            ...
            CancellationToken ct = default)
        {
            lock (_gate)
            {
                if (!string.IsNullOrWhiteSpace(idempotencyScopeKey) && _idempotencyReservations.TryGetValue(idempotencyScopeKey, out var existingRunId))
                {
                    ...
                    return Task.FromResult(new GoogleCloudExecutionRunCreation
                    {
                        Created = false,
                        RunId = existing.Id,
        ...
        public Task<GoogleCloudExecutionLeaseClaim?> TryClaimExternalRunAsync(string runId, ExecutionLeaseRequest request, CancellationToken ct = default)
        {
            lock (_gate)
            {
                if (!_runs.TryGetValue(runId, out var run)) return Task.FromResult<GoogleCloudExecutionLeaseClaim?>(null);
                var now = DateTime.UtcNow;
                _leases.TryGetValue(request.LeaseKey, out var existingLease);
                if (existingLease is not null && existingLease.ExpiresAtUtc > now) return Task.FromResult<GoogleCloudExecutionLeaseClaim?>(null);
                ...
                {
                    return Task.FromResult<GoogleCloudExecutionLeaseClaim?>(null);
                }
                ...
                _leases[lease.LeaseKey] = lease;
                return Task.FromResult<GoogleCloudExecutionLeaseClaim?>(new GoogleCloudExecutionLeaseClaim
                {
                    Run = Clone(run),
                    Lease = Clone(lease),
                    Recovered = recovered
        ...
        public Task<ExecutionRun> SuspendExternalRunUnderLeaseAsync(string leaseKey, string ownerId, GoogleCloudExecutionWait wait, CancellationToken ct = default)
        {
            lock (_gate)
            {
        ...
        public Task<GoogleCloudExecutionExternalCompletion> CompleteExternalRunUnderLeaseAsync(string leaseKey, string ownerId, ExecutionRunResult result, CancellationToken ct = default)
        {
            lock (_gate)
            {
                ...
                if (IsCompletedLease(lease) && (ExecutionRunStatuses.IsTerminal(run.Status) || run.Status == ExecutionRunStatuses.Waiting))
                    return Task.FromResult(new GoogleCloudExecutionExternalCompletion { Run = Clone(run), AlreadyCompleted = true });
                ...
                return Task.FromResult(new GoogleCloudExecutionExternalCompletion { Run = Clone(run), RetryScheduled = retryScheduled });
            }
        }
        ...
        public Task<GoogleCloudExecutionWait?> GetWaitAsync(string runId, CancellationToken ct = default)
        {
        ...
        public Task<GoogleCloudExecutionRunDeletion> DeleteRunAsync(ExecutionRun run, CancellationToken ct = default)
        {
            lock (_gate)
            {
                _runs.Remove(run.Id);
                ...
                var result = new GoogleCloudExecutionRunDeletion
                {
                    RunId = run.Id,
                    Runs = 1,
